...
download_and_extract_backhaul() {
    ...
    case "$ARCH" in
        x86_64)
            DOWNLOAD_URL="https://raw.githubusercontent.com/niiiii/mmdrz-backhaul/refs/heads/main/backhaul_amd64.tar.gz"
            ;;
        arm64|aarch64)
            DOWNLOAD_URL="https://raw.githubusercontent.com/niiiii/mmdrz-backhaul/refs/heads/main/backhaul_arm64.tar.gz"
            ;;
        *)
...
EOF
    ...
    echo -e "Telegram Channel: ${YELLOW}@mmcloudvpn${NC}"
}
...
if [[ ! -d "$config_dir" ]]; then
    ...
    echo
    colorize green "1) IRAN server" bold
    colorize magenta "2) KHAREJ server" bold
    echo
...
iran_server_configuration() {  
    ...
    cat << EOF > "${config_dir}/mmdrzIR${tunnel_port}.toml"
[server]
...
EOF
	...
	for port in "${ports[@]}"; do
	    if [[ "$port" =~ ^[0-9]+-[0-9]+$ ]]; then
	        ...
	        echo "    \"$port\"," >> "${config_dir}/mmdrzIR${tunnel_port}.toml"
	...
	echo "]" >> "${config_dir}/mmdrzIR${tunnel_port}.toml"
	
	echo
	
	colorize green "generated successfully!  (MMDRZ)(@mmcloudvpn)"
...
[Service]
Type=simple
ExecStart=${config_dir}/backhaul_premium -c ${config_dir}/mmdrzIR${tunnel_port}.toml
Restart=always
RestartSec=3
